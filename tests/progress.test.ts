...
import { coursePercent, summarizeWeeks, weekProgressForSlug } from "../src/server/progress";
...
describe("progress summary", () => {
  ...
  it("weekProgressForSlug returns one week row for persist path", () => {
    const week = weeks[1];
    const state = {
      lessons: week.lessons.slice(0, 1).map((lesson) => ({
        lessonId: lesson.id,
        completedAt: new Date(),
      })),
      labs: [],
    ...
    const row = weekProgressForSlug(state, week.slug);
    assert.ok(row);
    assert.equal(row.week.slug, week.slug);
    assert.ok(row.percent > 0);
