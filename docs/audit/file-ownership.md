...
Один файл на агента. Вторую карточку решения не добавлять. Глоссарий, types, index, curriculum, тесты и docs не трогать. Новых недель нет. Если тема уже измеряется в лаборатории, файл не менять и написать ALREADY_PRESENT.

| Agent | Owned files | Тема |
| ----- | ----------- | ---- |
| M1 | `course/weeks/week-02.ts` | Локальная модель: промпт не уходит к провайдеру, что замерить |
| M2 | `course/weeks/week-32.ts` | Serving: один запрос, пакет, холодный старт, таймаут. Без Kubernetes |
| M3 | `course/weeks/week-25.ts` | Решение prompt / RAG / fine-tune. Обучение весов не запускать |
| M4 | `course/weeks/week-10.ts` | Маршрут дешёвой и сильной модели и своя стоимость |
| M5 | `course/weeks/week-24.ts` | Таксономия reliability, security, safety, privacy. Если уже есть, не дублировать |
