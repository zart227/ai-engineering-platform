...
| P0-EXPORT | Полный backup learner state | READY_FOR_VERIFICATION | `buildExport` пишет formatVersion 3, включая `RecallReview` (weekSlug, itemIndex, prompt, nextReviewAt, reviewCount). Импорт принимает v2 и v3. v2 расписание не стирает, и превью это говорит. v3, включая ноль карточек, в превью предупреждает о полной замене. Roundtrip интервалов 1, 3, 7, 21 дня: `tests/export.test.ts` | Agent B | — | Ждёт проверку оркестратора |
| P0-IMPORT | Validate → version → migrate → preview → transaction → result | DONE | `migrateExport`, `previewImport`, `$transaction`. v1 не удаляет квизы и события. После транзакции импорт не переписывает week progress и не дописывает week_opened или week_completed | Agent B | P0-EXPORT | Закрыто в Wave 1 |
...
| C-MISSING | Local models, serving, fine-tune, routing, privacy taxonomy | READY_FOR_VERIFICATION | M1 неделя 2: llama.cpp как движок, Ollama как демон, GGUF, замер одного prompt hosted и local, выбор в рубрике. M2 неделя 32: Request → Scheduler → Batch → Model → Token и выбор latency или throughput, отдельно от шага клиента. M3 неделя 25: SFT, LoRA, QLoRA, instruction tuning, preference optimization, три строки рычагов в рубрике. M4 неделя 10: fallback provider routing. M5 неделя 24 не ломалась. GATE 4 открыт | M1–M5 | GATE 3 | Ждёт оркестратора |
...
| P1-ANALYTICS | Funnel по LearningEvent | READY_FOR_VERIFICATION | Единица user-week. Порядок: week_opened → lesson_started → lesson_completed → practice_completed → quiz_passed → artifact_completed → week_completed. Поздний тип не обгоняет предыдущий шаг. Конверсия и отсев в процентах, отсев ещё и числом | Wave 5.1 | — | Ждёт проверку |
| P3-SEARCH | Хеш-поиск ученика | READY_FOR_VERIFICATION | pgvector и `CourseChunk` остаются. Косинус `<=>` по вектору `feature-hash-v1`: хеш слов, префиксов и биграмм, 384 измерения. Перефраз без общих слов не находится. Страница `/search` и `course.search` называют это хеш-поиском | Wave 5.1 | Use case подтверждён | Ждёт проверку |
