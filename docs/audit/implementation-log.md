...
На этом main есть поля контракта недели и `recall`. В глоссарии нет `agent-budget` и `recall-at-k`. В неделе 2 нет шага локальной модели, в неделе 10 нет шага маршрута модели. Чекбоксы Wave 4 и GATE 4 не закрыты.
...
PASS для P1 и P2. P3 и P4 остаются открытыми.
